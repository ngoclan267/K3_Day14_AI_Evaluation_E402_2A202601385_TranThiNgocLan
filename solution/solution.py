...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        if not answer_tokens:
            return 1.0  # If answer is empty, consider it fully faithful
        overlap = answer_tokens.intersection(context_tokens)
        faithfulness_score = len(overlap) / len(answer_tokens)
        clamped_score = max(0.0, min(1.0, faithfulness_score))
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        if not question_tokens:
            return 1.0  # If question is empty, consider answer fully relevant
        overlap = answer_tokens.intersection(question_tokens)
        relevance_score = len(overlap) / len(question_tokens)
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        # TODO
        answer_tokens = _tokenize(answer)
        completeness_tokens = _tokenize(expected)
        completeness_score = 1.0 if not completeness_tokens else len(answer_tokens.intersection(completeness_tokens)) / len(completeness_tokens)
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        recall_score = len(expected_tokens.intersection(union_tokens)) / len(expected_tokens)
    ...
    def run_full_eval(
        ...
        passed = faithfulness_score >= 0.5 and relevance_score >= 0.5 and completeness_score >= 0.5
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            ...
            entry_averages.append(sum(entry_scores) / len(entry_scores) if entry_scores else 0.0)

        overall_average = sum(entry_averages) / len(entry_averages)

        positional_bias = False
        if len(entry_averages) >= 2:
            first_avg = entry_averages[0]
            rest_avg = entry_averages[1:]
            positional_bias = all(first_avg > avg for avg in rest_avg)
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if low_scores == ["faithfulness"]:
            return "Context is missing or irrelevant — improve retrieval"
        if low_scores == ["relevance"]:
            return "Answer does not address the question — improve prompt clarity"
        if low_scores == ["completeness"]:
            return "Answer is missing key information — increase context window or improve generation"

        # No single score under 0.5 (e.g. borderline failure) — fall back to the lowest of the three.
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            lines.append(
                f"| {failure_id} | {failure.failure_type} | {root_cause} | {suggested_fix} | Open |"
            )
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Add few-shot examples showing complete, on-topic answers"
            )

        generic_suggestions = [
            "Expand the golden dataset with more cases similar to the observed failures",
            "Add regression tests to prevent these failure patterns from recurring",
